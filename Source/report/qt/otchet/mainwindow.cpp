	...
	, progress(0), ui(new Ui::MainWindow)
{
	...
	connect(&calendar, &Calendar::finished, this, &MainWindow::another_window_close);
	 //connect(&parser, &Parser::log_message, this, &MainWindow::message_to_log);
	 //connect(&pdf_creator, &PDF_creator::log_message, this, &MainWindow::message_to_log);
	connect(&pdf_creator, &PDF_creator::log_message, [&](auto& message, auto color) {
		ui->loger->addLog(message, 0, color);
		save_log.log(message); });
	connect(&parser, &Parser::log_message, [&](auto &message,auto color) {
		ui->loger->addLog(message, 0, color);
		save_log.log(message); });
...
void MainWindow::message_to_log(const QString& message,QRgb color)
{
	ui->loger->addLog(message,0,color);
	save_log.log(message);
}
